...
export default function Despesas() {
  ...
  return (
    <div className="space-y-6 pb-2">
      ...
      {listFilter === 'proximos' && (
        <div className="flex flex-col gap-2 rounded-xl border border-rose-400/35 bg-rose-500/10 px-3 py-3 text-sm sm:flex-row sm:items-center sm:justify-between">
          <span>
      ...
      {listActionMsg && (
        <p className="rounded-md border border-rose-500/35 bg-rose-500/10 px-3 py-2 text-sm text-rose-100">{listActionMsg}</p>
      )}

      <Card className="border-rose-500/25 shadow-md shadow-rose-500/5">
        <CardHeader className="rounded-t-xl border-b border-rose-500/15 bg-gradient-to-r from-rose-500/15 to-violet-500/10">
          <CardTitle className="text-base text-rose-100">Filtros da lista</CardTitle>
        </CardHeader>
      ...
      <Card className="border-rose-500/20 shadow-lg shadow-rose-500/5">
        <CardHeader className="rounded-t-xl border-b border-rose-500/10 bg-gradient-to-r from-rose-500/12 to-transparent">
          <CardTitle className="text-base text-rose-50">Nova despesa</CardTitle>
        </CardHeader>
      ...
      <Card className="border-violet-500/20 shadow-lg shadow-violet-500/5">
        <CardHeader className="rounded-t-xl border-b border-violet-500/10 bg-gradient-to-r from-violet-500/12 to-transparent">
            ...
            <Table>
              ...
              <TBody>
                {displayRows.slice(0, 80).map((r) => (
                  <TR
                    key={r.id}
                    className={cn('cursor-pointer transition-colors hover:bg-rose-500/10')}
                    onClick={() => openEdit(r)}
                  >
                    <TD className="text-center" onClick={(e) => e.stopPropagation()}>
                      {r.status === 'PREVISTO' || r.status === 'ATRASADO' ? (
                        <Button
                          ...
                          size="icon"
                          variant="outline"
                          className="h-9 w-9 touch-manipulation border-rose-500/45"
                    ...
                    <TD className="font-medium text-rose-200">{brl(parseFloat(r.amount))}</TD>
          ...
          <ul className="space-y-2 md:hidden">
            {displayRows.slice(0, 80).map((r) => (
              <li key={r.id} className="flex gap-2">
                <button
                  type="button"
                  className="min-w-0 flex-1 rounded-xl border border-rose-500/25 bg-gradient-to-br from-rose-500/10 to-card px-3 py-3 text-left text-sm shadow-md transition hover:border-rose-400/40"
                  onClick={() => openEdit(r)}
                >
                  <p className="font-medium leading-snug text-rose-50">{r.description}</p>
                  ...
                  <p className="mt-2 flex flex-wrap items-center justify-between gap-2">
                    <span className="text-base font-semibold text-rose-200">{brl(parseFloat(r.amount))}</span>
                ...
                {(r.status === 'PREVISTO' || r.status === 'ATRASADO') && (
                  <Button
                    ...
                    size="icon"
                    variant="outline"
                    className="h-11 w-11 shrink-0 self-center touch-manipulation border-rose-500/45"
                    title="Pago"
