...
export default function Membros() {
  ...
  async function load() {
    const { data } = await api.get<Row[]>('/household-members');
    setRows(data);
  }
  ...
  async function create(e: React.FormEvent) {
    e.preventDefault();
    await api.post('/household-members', { name });
    setName('');
    await load();
  }
  ...
  async function saveEdit(ev: React.FormEvent) {
    ...
    setSaving(true);
    try {
      await api.patch(`/household-members/${editing.id}`, { name: eName, active: eActive });
      setEditOpen(false);
      setEditing(null);
      await load();
    } finally {
    ...
    <div className="space-y-6">
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="max-w-md" onOpenAutoFocus={(ev) => ev.preventDefault()}>
          <h2 className="text-lg font-semibold">Editar membro / originador</h2>
          {editing && (
            <form onSubmit={saveEdit} className="grid gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <Label htmlFor="mem-active" className="font-normal">
                  Ativo
                </Label>
              </div>
      ...
      <h1 className="text-2xl font-semibold">Membros / originadores</h1>
      <Card>
        <CardHeader>
