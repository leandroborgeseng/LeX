...
# SQLite: diretório precisa existir e ser gravável (volume Railway em /data)
DATA_DIR="${LEX_DATA_DIR:-/data}"
mkdir -p "$DATA_DIR"

if ! touch "$DATA_DIR/.lex-write-test" 2>/dev/null; then
  echo "LeX: ERRO — '$DATA_DIR' não é gravável. No Railway, monte um volume persistente em /data (ou defina LEX_DATA_DIR)." >&2
  exit 1
fi
...
# file:/absoluto → file:///absoluto (SQLite/Prisma no Linux)
